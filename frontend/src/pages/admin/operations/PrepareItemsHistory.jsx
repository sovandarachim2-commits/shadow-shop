...
import {
  ...
  Image as ImageIcon, ClipboardList, AlertCircle,
} from 'lucide-react'
...
function getSetQrValues(record) {
  if (record.set_type !== 'set') return []
  return Array.isArray(record.set_qr_values)
    ? record.set_qr_values.map((value) => String(value).trim()).filter(Boolean)
    : []
}
...
function findSetNameByQr(qrValue, productSets) {
  const normalizedQr = normalizeLookupValue(qrValue)
  if (!normalizedQr) return '-'
  const match = productSets.find((item) => {
    const itemCandidates = (item.items || []).flatMap((setItem) => [
      setItem.product,
      setItem.product_code,
      setItem.product_name,
    ])
    const candidates = [item.id, item.name, item.slug, ...itemCandidates].map(normalizeLookupValue)
    return candidates.includes(normalizedQr)
  })
  return match?.name || '-'
}

function NumberedListCell({ values }) {
...
export default function PrepareItemsHistory() {
  ...
  const { data: productSets = [] } = useQuery({
    queryKey: ['prepare-history-product-sets'],
    queryFn: () => productsApi.sets.list({ page_size: 200 }).then((r) => getListResults(r.data)),
    staleTime: 60000,
  })

  const { data, isLoading, refetch, isFetching } = useQuery({
    queryKey: ['prepare-history', search, dateFrom, dateTo, limit],
    queryFn: async () => {
      const params = {
      ...
      const [orderResponse, manualResponse] = await Promise.allSettled([
        ordersApi.orders.list({ ...params, status: 'preparing' }),
  ...
  const orderRows = getListResults(data?.orders).map((order) => ({
    ...
    invoice_photo: order.prepare_invoice_photo,
    package_photo: order.prepare_package_photo,
    user_name: order.status_changed_by_name,
    date_time: order.status_changed_at || order.updated_at || order.created_at,
    set_qr_values: [],
    set_name_values: order.preview_name ? [order.preview_name] : [],
  }))
  const manualRows = getListResults(data?.manual).map((record) => ({
    ...
    set_qr_values: getSetQrValues(record),
    set_name_values: getSetQrValues(record).map((value) => findSetNameByQr(value, productSets)),
  }))
  ...
  const setNames = [
    ...new Set(orders.flatMap((order) => order.set_name_values || []).filter((name) => name && name !== '-')),
  ]
  ...
  const handleDelete = async (row) => {
    ...
    try {
      ...
      toast.success('Prepare package record deleted')
      refetch()
    } catch {
      toast.error('Failed to delete prepare package record')
    }
  }
