...
function transactionMeta(item) {
  ...
  if (item.order_number) return { icon: ShoppingBag, title: `Order #${item.order_number}`, tone: 'bg-emerald-50 text-emerald-600' }
  if (note.includes('check-in')) return { icon: CalendarCheck, title: 'Daily Check-in', tone: 'bg-emerald-50 text-emerald-600' }
  if (note.includes('review')) return { icon: Edit3, title: 'Write a Review', tone: 'bg-emerald-50 text-emerald-600' }
  if (note.includes('referral') || note.includes('friend')) return { icon: UserPlus, title: 'Referral Bonus', tone: 'bg-pink-50 text-pink-600' }
  if (note.includes('birthday')) return { icon: Gift, title: 'Birthday Bonus', tone: 'bg-amber-50 text-amber-600' }
  if (Number(item.points) < 0) return { icon: Ticket, title: item.note || 'Reward redeemed', tone: 'bg-violet-50 text-violet-600' }
  return { icon: Star, title: item.note || 'Points earned', tone: 'bg-pink-50 text-pink-600' }
}
...
export default function PointsHistory() {
  ...
  if (isError || historyError) return <div className="py-20 text-center text-sm font-bold text-gray-500">Points history could not load.</div>
      ...
      <div className="mx-auto w-full max-w-[560px] px-4 md:max-w-[1440px] md:px-6 md:pt-6">
        <header className="sticky top-0 z-30 -mx-4 grid min-h-[60px] grid-cols-[44px_1fr_44px] items-center bg-white/95 px-4 pt-[env(safe-area-inset-top)] backdrop-blur md:static md:mx-0 md:mb-4 md:flex md:min-h-0 md:items-start md:justify-between md:bg-transparent md:px-0 md:pt-0">
          ...
          <div className="min-w-0 text-center md:text-left">
            <h1 className="text-lg font-black text-gray-950 md:text-2xl">Points History</h1>
            <p className="mt-1 hidden text-xs font-semibold text-gray-500 md:block">Track every point earned, used, and expired.</p>
          </div>
        ...
        <h2 className="mb-3 mt-6 text-lg font-black text-gray-950">How You Earn Points</h2>
          ...
          {FILTERS.map((filter) => (
            <button
              ...
              className={cn(
                ...
                activeFilter === filter.key ? 'border-pink-600 bg-pink-600 text-white' : 'border-gray-200 bg-white text-gray-500'
              )}
            >
              {filter.label}
            </button>
        ...
        <section className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm">
              ...
              <div key={item.id} className="flex min-h-[76px] items-center gap-3 border-b border-gray-100 px-3 py-3 last:border-b-0">
                ...
                <div className="shrink-0 text-right">
                  <p className={cn('text-sm font-black', expired ? 'text-gray-400' : pointsValue >= 0 ? 'text-emerald-600' : 'text-red-500')}>
                    {pointsValue > 0 ? '+' : ''}{pointsValue.toLocaleString()} pts
                  </p>
                  <p className="mt-1 text-[11px] font-semibold text-gray-400">{expired ? 'Expired' : formatTransactionTime(item.created_at)}</p>
                </div>
              </div>
            )
          })}
          {filteredTransactions.length === 0 && (
            <div className="py-14 text-center">
              ...
              <p className="mt-3 text-sm font-black text-gray-500">No {activeFilter === 'all' ? '' : activeFilter} point activity</p>
            </div>
          )}
        </section>
        ...
        <section className="mt-5 flex items-center justify-between rounded-2xl bg-gradient-to-r from-pink-50 to-rose-50 p-4">
          <div>
            <h2 className="text-sm font-black text-gray-950">Keep earning points</h2>
            <p className="mt-1 text-xs font-semibold text-gray-600">Enjoy more rewards with every order.</p>
            <button type="button" onClick={() => navigate('/profile/rewards/redeem')} className="mt-3 rounded-lg bg-pink-600 px-4 py-2 text-xs font-black text-white">Explore Rewards</button>
          </div>
