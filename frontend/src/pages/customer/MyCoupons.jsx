...
const FILTER_KEYS = ['all', 'active', 'used', 'expired']
...
function couponVisual(coupon, t) {
  if (coupon.reward_type === 'free_delivery') return { icon: Truck, label: t('rewardsPage.coupons.freeShip'), sub: t('rewardsPage.coupons.ship'), tone: 'from-sky-500 to-cyan-500' }
  if (coupon.reward_type === 'manual') return { icon: Gift, label: t('rewardsPage.coupons.gift'), sub: t('rewardsPage.coupons.giftCoupon'), tone: 'from-violet-500 to-fuchsia-500' }
...
function expiryText(coupon, t) {
  if (!coupon.ends_at) return t('rewardsPage.coupons.noExpiry')
  return formatDate(coupon.ends_at)
}

function CouponCard({ coupon, onApply, t }) {
      ...
      <div className="grid grid-cols-[72px_1fr_auto] gap-3">
        ...
        <div className="min-w-0 py-1">
          ...
          <p className="mt-1 text-xs font-semibold text-gray-500">
            {t('rewardsPage.coupons.minOrder', { amount: Number(coupon.minimum_order_amount || 0).toFixed(0) })}
          </p>
          <p className="mt-2 inline-flex rounded-md bg-pink-50 px-2 py-1 text-[11px] font-black text-pink-600">
            {t('rewardsPage.coupons.codeLabel', { code: coupon.coupon_code })}
          </p>
        </div>
        ...
        <div className="flex min-w-[76px] flex-col items-end justify-between">
          <div className="text-right">
            <p className={cn('text-xs font-black capitalize', active ? 'text-emerald-600' : status === 'used' ? 'text-gray-500' : 'text-gray-400')}>
              {t(`rewardsPage.status.${status}`)}
            </p>
            <p className="mt-1 text-[10px] font-semibold leading-tight text-gray-400">
              {status === 'used' ? t('rewardsPage.status.used') : status === 'expired' ? t('rewardsPage.status.expired') : t('rewardsPage.coupons.expiresOn')}<br />
              {expiryText(coupon, t)}
            </p>
          </div>
          <button
          ...
          >
            {active ? t('rewardsPage.coupons.apply') : status === 'used' ? t('rewardsPage.status.used') : t('rewardsPage.status.expired')}
          </button>
        </div>
      </div>
    </article>
...
export default function MyCoupons() {
  ...
  const handleApply = (coupon) => {
    ...
    toast.success(t('rewardsPage.toast.couponApplied', { code: coupon.coupon_code }))
    navigate('/cart')
  }
      ...
      <div className="mx-auto w-full max-w-[560px] px-4 md:max-w-[1440px] md:px-6 md:pt-6">
          ...
          {FILTER_KEYS.map((key) => (
            <button
              ...
              className={cn(
                ...
                activeFilter === key ? 'bg-pink-600 text-white shadow-sm shadow-pink-100' : 'bg-gray-100 text-gray-500'
              )}
            >
        ...
        {filteredCoupons.length === 0 ? (
          ...
          <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            {filteredCoupons.map((coupon) => <CouponCard key={coupon.id} coupon={coupon} onApply={handleApply} t={t} />)}
          </section>
        )}
      </div>
    </div>
