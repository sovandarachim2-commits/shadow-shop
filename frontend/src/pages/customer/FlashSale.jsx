...
function FlashProductCard({ product, priority = false }) {
  ...
  return (
    ...
    >
      ...
      {discountPct > 0 && (
        <div className="absolute right-3 top-3 z-10 rounded-full bg-pink-600 px-3.5 py-1.5 text-xs font-black leading-none text-white shadow-lg shadow-pink-200 ring-2 ring-white">
          -{discountPct}%
        </div>
      )}
      ...
      <div className="p-3">
        ...
        <div className="mt-2 flex items-center justify-between gap-2">
          <span className="text-[11px] font-black uppercase tracking-wide text-gray-400">
            {maxOrderQty > 0 ? `Max ${maxOrderQty}` : available ? t('common.inStock') : t('common.outOfStock')}
          </span>
          {!available ? (
